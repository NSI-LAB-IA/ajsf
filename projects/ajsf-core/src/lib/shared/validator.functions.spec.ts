import { toIsoString, toJavaScriptType, toSchemaType } from "./validator.functions";

describe("Validator functions", () => {
  describe("toIsoString", () => {
    it("should work with timezone", () => {
      expect(toIsoString(new Date("05 October 2011 14:48 UTC"))).toEqual("2011-10-05");
    });
  });
  ...
  describe("toSchemaType", () => {
    ...
    it("Boolean conversion examples", () => {
      expect(toSchemaType("1", ["integer", "number", "string", "boolean"])).toEqual("1");
      expect(toSchemaType("1", ["string", "boolean"])).toEqual("1");
      expect(toSchemaType("1", ["boolean"])).toEqual("1");
      expect(toSchemaType("true", ["number", "string", "boolean"])).toEqual("true");
      expect(toSchemaType("true", ["boolean"])).toEqual("true");
      expect(toSchemaType("true", ["number"])).toEqual(0);
    });
    it("string conversion examples", () => {
      expect(toSchemaType("1.58", ["boolean", "number", "integer", "string"])).toEqual("1.58");
      expect(toSchemaType("1.5", ["boolean", "number", "integer"])).toEqual("1.5");
    });
  });
});
